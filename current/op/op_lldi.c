...
/*   Updated: 2019/01/05 01:06:51 by smonroe          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	op_lldi(t_cyc *info, t_pc *pc)
{
	...
	if ((acb & 0x30) == (REG_CODE << 4))
		...
		cw_memr(&tmp, &info->mem[0][MEM(pc->i + 2 + ACB_ARG((acb & 0x30) >> 4))], IND_SIZE);
	loc += tmp;
	tmp = info->mem[0][MEM(pc->i + acb_len(acb) - 1)].byte;
	//ft_printf("%d as loc and for tmp %d\n", loc, tmp);
	if (REG(tmp))
		cw_memr(&pc->r[tmp], &info->mem[0][MEM(pc->i + (int16_t)loc)], REG_SIZE);
	pc->i += acb_len(acb);
